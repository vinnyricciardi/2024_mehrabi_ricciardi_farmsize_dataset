...
<<trainlesiverroranalysis, echo=FALSE, include=FALSE>>=
# tmp_pred <- kknn::kknn(field_size ~ .,
#                        train = fs_train,
#                        test = fs_train,
...
#
# tmp_fitted <- fs_train
# tmp_fitted$pred <- fitted(tmp_pred)
#
# tmp_fitted %>%
#   mutate(
#     match = field_size == pred,
#     overall_accuracy = sum(match) / n()
#     ) %>%
#   group_by(field_size) %>%
#   summarise(
#     tot = n(),
#     p_match = sum(match) / tot,
#     p_match_mis = 1 - p_match,
#     overall_accuracy = max(overall_accuracy)
#     )
#
# tmp_fitted %>%
#   mutate(
#     field_size = ifelse(field_size %in% c('very small', 'small'), 'small',
#                         ifelse(field_size %in% c('very large', 'large'), 'large',
#                         'medium')),
#     pred = ifelse(pred %in% c('very small', 'small'), 'small',
#                         ifelse(pred %in% c('very large', 'large'), 'large',
#                         'medium')),
#     match = field_size == pred,
#     overall_accuracy = sum(match) / n()
#     ) %>%
...
tmp_compare <- tmp_compare %>%
  ungroup() %>%
  mutate(fs = as.numeric(str_split(variable, '_', simplify = T)[,1])) %>%
  dplyr::arrange(fs) %>%
  mutate(variable = factor(variable, levels = unique(variable)))

# plot comparison
p <- ggplot(tmp_compare, aes(value.xp, value.yp)) +
  geom_point(aes(color = variable, group = country, size = totx)) +
  geom_abline(aes(slope = 1, intercept = 0)) +
  xlab('Our data') +
  ylab("Lowder's data") +
  labs(color = 'Farm Size (ha)', size = 'Total Area (ha)') +
...
tmp_compare %>%
  select(country, variable, value.xp, value.yp, diff) %>%
  filter(diff >= .05) %>%
  arrange(desc(diff)) %>%
  print(n = 100)
...
<<esdacheck, fig.cap='ESDA Checks', out.width='0.6\\linewidth', fig.height=5, fig.width=7, size='footnotesize', echo=F, fig=T>>=
tmp_compare %>%
  group_by(country) %>%
  summarise(
    totx = sum(value.x),
  ...
  mutate(
    diff = totx - toty,
    abs_diff = abs(diff),
    p_diff = (totx - toty) / totx
  ) %>%
  arrange(desc(p_diff)) %>%
  mutate(country = factor(country, unique(country))) %>%
  ggplot(aes(p_diff, country)) +
  geom_col()
@
...
# fts <- list.files(path = 'raster_out_forCleanRunTest/',
                 # pattern = '_1.tif$',
                 full.names = T)
# fs <- raster(fts)
...
fs <- calc(fs, getmode)
fs <- focal(fs, matrix(1,3,3), modal)
writeRaster(fs, "mode_farmsize.tif", overwrite = TRUE)
fs_spdf <- as(fs, 'SpatialPixelsDataFrame')
